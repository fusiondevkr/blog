...
앞서 [파워 앱][pas]을 이용해 굿즈 신청 화면까지 만들어 봤습니다. 파워 앱 안에서 신청 버튼을 탭할 경우 [파워 오토메이트][pau] 워크플로우를 실행시켜 굿즈 신청 프로세스를 진행합니다. 여기서는 이 파워 오토메이트 워크플로우를 만들어 보겠습니다.
...
2. 인스턴트 클라우드 흐름 작성 팝업 창이 뜨면 흐름 이름을 "**굿즈 신청 플로우**"라고 적어주고, 그 아래 "**이 흐름의 트리거 방법 선택**"에서 "**PowerApps**"를 선택합니다. 이후 "**만들기**" 버튼을 클릭해서 다음 화면으로 넘어갑니다.
...
8. 파워 오토메이트 워크플로우로 돌아가서 새 액션을 추가합니다. "**logistics**"로 검색하면 방금 만든 커스텀 커넥터가 보입니다. "**Run shipping queue**" 액션을 추가합니다.

    ![Run shipping queue 액션 선택][image-52]

9. 아래의 내용과 같이 "**name**", "**email**", "**address**", "**items**" 필드에 각각 입력합니다.
    ...
    * **items**: `body('JSON_구문_분석')?['items']`
    ...
    ![Run shipping queue 액션 항목 입력][image-53]
...
2. "**email**", "**checkpoint**" 필드에 아래와 같이 입력하세요.
    ...
    * **checkpoint**: `3`
    ...
    ![Run check-in 액션 항목 입력][image-55]
